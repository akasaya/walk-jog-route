...
- [x] 11.1 `RouteCard` コンポーネントを実装する（実施日時・移動モード・計画距離・実績記録の有無を表示）
- [x] 11.2 `History` ページを実装する（ルート一覧 → 選択 → `RouteMap` でポリライン表示の連携）
...
- [x] 12.1 Pydantic `RouteRequest` の `Field` 範囲制約（lat: -90〜90、lon: -180〜180、distance_km: 0.5〜50）と 422 レスポンスを統合テストで確認する
- [x] 12.2 ログ出力で GPS 座標を `round(lat, 1), round(lon, 1)` に丸めていることを確認し、必要な箇所に追加する
