...
def services():
    ...
    routing_svc.generate_round_trip = AsyncMock(return_value=("encoded_poly", 5000, 60))

    return weather_svc, history_svc, routing_svc


@pytest.fixture
def route_service(services):
    weather_svc, history_svc, routing_svc = services
    return RouteService(
...
@pytest.mark.asyncio
async def test_suggest_calls_round_trip_with_target_distance_m(services):
    weather_svc, history_svc, routing_svc = services
    svc = RouteService(
        weather_service=weather_svc,
        history_service=history_svc,
    ...
    routing_svc.generate_round_trip.assert_called_once()
    call_args = routing_svc.generate_round_trip.call_args
    assert call_args.args[2] == 5000  # target_distance_m = 5.0km * 1000
